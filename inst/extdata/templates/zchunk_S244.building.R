...
module_energy_S244.building <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L201.GDP_SEA <- get_data(all_data, "L201.GDP_SEA")
    ...
    S244.DeleteConsumer_SEAbld <- tibble(region = "Thailand", gcam.consumer = unique(L244.SubregionalShares$gcam.consumer))
    S244.DeleteSupplysector_SEAbld <- tibble(region = "Thailand", supplysector = unique(L244.Supplysector_bld$supplysector))
    ...
    write_to_Bangkok_RoT <- function(data){
      data_new <- subset(data, region == "Thailand") %>%
        repeat_add_columns(tibble(new_region = c("Bangkok", "Rest of Thailand"))) %>%
        mutate(region = new_region) %>%
        select(-new_region)
      return(data_new)
    }

    S244.list_nochange_data <- lapply(S244.list_nochange_data, FUN = write_to_Bangkok_RoT)

    S244.pop_gdp_share <- L201.Pop_SEA %>%
      left_join_error_no_match(L201.GDP_SEA, by = c("region", "year")) %>%
      group_by(year) %>%
    ...
    downscale_to_Bangkok_RoT <- function(data, share_data, value.column = "value", share.column = "popshare"){
      data_new <- subset(data, region == "Thailand") %>%
        repeat_add_columns(tibble(new_region = c("Bangkok", "Rest of Thailand"))) %>%
        mutate(region = new_region) %>%
        select(-new_region) %>%
    ...
    S244.Floorspace <- L244.Floorspace %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "base.building.size", share.column = "popshare")

    S244.ThermalBaseService <- L244.ThermalBaseService %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "base.service", share.column = "gdpshare")

    S244.GenericBaseService <- L244.GenericBaseService %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "base.service", share.column = "gdpshare")

    S244.StubTechCalInput_bld <- L244.StubTechCalInput_bld %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "calibrated.value", share.column = "gdpshare")

    S244.pol_emissions_bld_SEA <- L201.en_pol_emissions %>%
      filter(supplysector %in% L244.Supplysector_bld$supplysector) %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "input.emissions", share.column = "gdpshare")

    S244.ghg_emissions_bld_SEA <- L201.en_ghg_emissions %>%
      filter(supplysector %in% L244.Supplysector_bld$supplysector) %>%
      downscale_to_Bangkok_RoT(share_data = S244.pop_gdp_share, value.column = "input.emissions", share.column = "gdpshare")
    ...
    S244.DeleteConsumer_SEAbld %>%
      add_title("Delete gcam-consumer objects for buildings sector in Thailand") %>%
      add_units("Unitless") %>%
    ...
    S244.DeleteSupplysector_SEAbld %>%
      add_title("Delete supplysector objects for buildings sector in Thailand") %>%
      add_units("Unitless") %>%
      add_comments("Thailand region name hard-wired") %>%
    ...
    S244.Floorspace %>%
      ...
      add_precursors("L244.Floorspace", "L201.Pop_SEA", "L201.GDP_SEA") ->
      S244.Floorspace
    ...
    S244.ThermalBaseService %>%
      ...
      add_precursors("L244.ThermalBaseService", "L201.Pop_SEA", "L201.GDP_SEA") ->
      S244.ThermalBaseService
    ...
    S244.GenericBaseService %>%
      ...
      add_units("EJ/yr") %>%
      add_comments("L144.base_service_EJ_serv rounded and renamed") %>%
      add_precursors("L244.GenericBaseService", "L201.Pop_SEA", "L201.GDP_SEA") ->
      S244.GenericBaseService
    ...
    S244.StubTechCalInput_bld %>%
      ...
      add_precursors("L244.StubTechCalInput_bld", "L201.Pop_SEA", "L201.GDP_SEA") ->
      S244.StubTechCalInput_bld
    ...
    S244.list_nochange_data[["L244.HDDCDD_constdd_no_GCM"]] %>%
      add_title("HDD and CDD (constant)") %>%
    ...
    S244.list_nochange_data[["L241.fgas_all_units"]] %>%
      add_title("Units of f-gas emissions from buildings in SEAsia") %>%
    ...
    S244.list_nochange_data[["L201.nonghg_max_reduction"]] %>%
      add_title("max-reduction of pollutant emissions from buildings in SEAsia") %>%
      add_units("Percent") %>%
      add_comments("Copied") %>%
    ...
    S244.list_nochange_data[["L201.nonghg_steepness"]] %>%
      add_title("steepness of pollutant emissions from buildings in SEAsia") %>%
      add_units("Unitless") %>%
      add_comments("Copied") %>%
    ...
    S244.list_nochange_data[["L241.hfc_future"]] %>%
      add_title("emissions coefficients of f-gases from buildings in SEAsia") %>%
    ...
    S244.pol_emissions_bld_SEA %>%
      add_title("pollutant emissions from SEAsia buildings") %>%
      add_units("Tg/yr") %>%
    ...
    S244.ghg_emissions_bld_SEA %>%
      add_title("greenhouse gas emissions from SEAsia buildings") %>%
      add_units("Tg/yr") %>%
      add_comments("downscaled from Thailand using GDP shares") %>%
      add_precursors("L201.en_ghg_emissions") ->
      S244.ghg_emissions_bld_SEA

    return_data(S244.DeleteConsumer_SEAbld,
                S244.DeleteSupplysector_SEAbld,
                S244.SubregionalShares,
                S244.PriceExp_IntGains,
                S244.Floorspace,
