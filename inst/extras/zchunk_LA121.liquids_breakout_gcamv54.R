...
module_energy_LA121.liquids <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    iso_GCAM_regID <- get_data(strip_attributes = TRUE, all_data, "common/iso_GCAM_regID")
    IIASA_biofuel_production <- get_data(strip_attributes = TRUE, all_data, "aglu/IIASA_biofuel_production")
    IIASA_biofuel_tech_mapping <- get_data(strip_attributes = TRUE, all_data, "aglu/IIASA_biofuel_tech_mapping")
    IIASA_biofuel_region_mapping <- get_data(strip_attributes = TRUE, all_data, "aglu/IIASA_biofuel_region_mapping")
    A_OilSeed_SecOut <- get_data(strip_attributes = TRUE, all_data, "aglu/A_OilSeed_SecOut")
    calibrated_techs <- get_data(strip_attributes = TRUE, all_data, "energy/calibrated_techs")
    IEA_product_rsrc <- get_data(strip_attributes = TRUE, all_data, "energy/mappings/IEA_product_rsrc")
    A21.unoil_demandshares <- get_data(strip_attributes = TRUE, all_data, "energy/A21.unoil_demandshares")
    A21.globaltech_coef <- get_data(strip_attributes = TRUE, all_data, "energy/A21.globaltech_coef")
    L100.IEA_en_bal_ctry_hist <- get_data(strip_attributes = TRUE, all_data, "L100.IEA_en_bal_ctry_hist")
    L1011.en_bal_EJ_R_Si_Fi_Yh <- get_data(strip_attributes = TRUE, all_data, "L1011.en_bal_EJ_R_Si_Fi_Yh")
    ...
    if(is.null(L100.IEA_en_bal_ctry_hist)) {
      ...
      L111.Prod_EJ_R_F_Yh <- L111.Prod_EJ_R_F_Yh <- get_data(strip_attributes = TRUE, all_data, "L111.Prod_EJ_R_F_Yh")
