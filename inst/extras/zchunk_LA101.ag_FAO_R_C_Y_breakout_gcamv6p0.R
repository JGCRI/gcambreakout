...
module_aglu_LA101.ag_FAO_R_C_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    regions_check <- GCAM_region_names$GCAM_region_ID[!GCAM_region_names$GCAM_region_ID %in% glu_gcam$GCAM_region_ID];  regions_check

    L101.ag_Prod_Mt_R_C_Y_GLU %>%
      dplyr::mutate(remove = dplyr::if_else(((GLU %in% remove_glu) & (GCAM_region_ID %in% regions_check)),1,0)) %>%
