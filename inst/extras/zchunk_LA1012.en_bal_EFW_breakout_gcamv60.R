...
module_energy_LA1012.en_bal_EFW <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # production technologies. These high estimates of deduction_EJ cause issues in the 'stop if' check below.

    # Mauritania - According to GPK, the error check can be done for 2015 only since this is the model calibration year. For Mauritania, the high
    # deduction_EJ values occur in the pre-2015 years only.
    AfW_Region_ID <- 5             # GCAM_region_ID of Africa_Western
    iso_GCAM_regID %>%
    ...
    if (mrt_Region_ID != AfW_Region_ID){
      ...
      L1012.en_bal_EJ_R_Si_Fi_Yh_desal %>%
        ...
        filter(GCAM_region_ID == mrt_Region_ID & year == 2015) -> L1012.en_bal_EJ_R_Si_Fi_Yh_desal_check_fraction

      # Make the error check only for Mauritania in 2015
    ...
    check_regions <- c("lbn", "kwt", "isr")
    Orig_GCAM_ID  <- c(21, 21, 21)
    idx           <- length(check_regions)

    for (index in 1:idx){
      iso_GCAM_regID %>%
        filter(iso == check_regions[index]) -> check_regions_tbl
      check_regions_Region_ID <- check_regions_tbl$GCAM_region_ID[1]

      # The lines below are executed only if any of the countries in the check_regions vector  has been broken out.
      if (check_regions_Region_ID != Orig_GCAM_ID[index]){
        L1012.en_bal_EJ_R_Si_Fi_Yh_desal %>%
          mutate(fraction = deduction_EJ / initial_energy_EJ) -> L1012.en_bal_EJ_R_Si_Fi_Yh_desal
        L1012.en_bal_EJ_R_Si_Fi_Yh_desal$fraction[is.na(L1012.en_bal_EJ_R_Si_Fi_Yh_desal$fraction)] <- 0

        L1012.en_bal_EJ_R_Si_Fi_Yh_desal$deduction_EJ <- ifelse(L1012.en_bal_EJ_R_Si_Fi_Yh_desal$fraction >= efw.MAX_COMMIND_ENERGY_DESAL,
                                                                efw.MAX_COMMIND_ENERGY_DESAL * L1012.en_bal_EJ_R_Si_Fi_Yh_desal$initial_energy_EJ,
                                                                L1012.en_bal_EJ_R_Si_Fi_Yh_desal$deduction_EJ)

        L1012.en_bal_EJ_R_Si_Fi_Yh_desal %>%
          select(-fraction) -> L1012.en_bal_EJ_R_Si_Fi_Yh_desal
      }
    }
    ###
