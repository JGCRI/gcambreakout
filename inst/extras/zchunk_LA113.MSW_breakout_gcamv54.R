...
module_energy_LA113.MSW <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # SRSdS, 25Aug21: Handling Western Sahara case.
    # Get GCAM_Region_ID of Western Africa
    iso_GCAM_regID %>%
      filter(iso == 'esh') -> esh_tbl
    esh_Region_ID <- esh_tbl$GCAM_region_ID[1]

    if (esh_Region_ID != 3){
      # Use Marocco data as an assumption for Western Sahara
      L100.gdp_mil90usd_ctry_Yh %>%
        filter(iso == 'mar') -> mar_tbl
      mar_tbl["iso"][mar_tbl["iso"] == "mar"] <- "esh"
      esh_tbl <- mar_tbl

      # Join new table to the original L100.gdp_mil90usd_ctry_Yh table
      rbind(L100.gdp_mil90usd_ctry_Yh, esh_tbl) -> L100.gdp_mil90usd_ctry_Yh_new

      # Sort by year
      L100.gdp_mil90usd_ctry_Yh_new[order(L100.gdp_mil90usd_ctry_Yh_new$year),] -> L100.gdp_mil90usd_ctry_Yh
    }
