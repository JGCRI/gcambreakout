...
#' @importFrom dplyr across bind_rows filter if_else group_by left_join mutate right_join select summarise summarise_at vars
...
module_aglu_LB165.ag_water_R_C_Y_GLU_irr <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    #### SRSdS, 19Aug21: Bahrein is not listed in the Rohwer_2007_IrrigationEff table. To allow being broken as an
    # inidividial region, it is assumed for Bahrein data corresponding to the mean values of neighboring countries.
    Rohwer_2007_IrrigationEff %>%
      filter(country == 'Saudi Arabia' | country == 'Qatar') %>%
      select(-country,  -iso) %>%
      summarise(across(where(is.numeric), ~ mean(.x))) %>%
      mutate(country = "Bahrain", iso = "bhr", .before = 1) -> mean_tbl
    Rohwer_2007_IrrigationEff <- rbind(Rohwer_2007_IrrigationEff, mean_tbl)
    ####
    ...
    ### SRSdS, 23Aug21: Handling Qatar case (only valid when Qatar is broken out individually and not together with Saudi Arabia)
    iso_GCAM_regID %>%
      filter(iso == 'qat') -> filtered_tbl_1
    iso_GCAM_regID %>%
      filter(iso == 'sau') -> filtered_tbl_2
    if (filtered_tbl_1$GCAM_region_ID != filtered_tbl_2$GCAM_region_ID){
      if (filtered_tbl_1$GCAM_region_ID != 21){
        # Get the GCAM_region_ID of Qatar
        filtered_tbl_1$GCAM_region_ID -> Qat_region_ID

        # Get values for the Middle East region
        L165.ag_IrrEff_R %>%
          filter(GCAM_region_ID == 21) -> MIE_tbl

        # Use data for the Middle East region as assumption for Qatar
        MIE_tbl$GCAM_region_ID <- Qat_region_ID
        rbind(L165.ag_IrrEff_R, MIE_tbl) -> L165.ag_IrrEff_R_tmp

        # Sort by GCAM_region_ID
        L165.ag_IrrEff_R_tmp[order(L165.ag_IrrEff_R_tmp$GCAM_region_ID),] -> L165.ag_IrrEff_R
      }
    }
    ###
