...
module_aglu_L202.an_input <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    ### SRSds, 24Aug21: Handling Iraq, Qatar and Oman cases when trying to break these countries as individual GCAM regions.
    # Adjusting the L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt table
    # Handling Qatar case.
    L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
      filter(region == 'Qatar') -> TMP
    reg_name <- TMP$region[1]
    check_NA <- is.na(reg_name)

    if (check_NA == FALSE){
      # Using Midlle East data as assumption for the broken out region.
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
        filter(region != reg_name) -> L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_tmp
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
        filter(region == 'Middle East') -> Mie_tbl

      # Get region ID for the broken out region
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
        filter(region == reg_name) -> Qat_tbl_tmp
      Qat_region_ID <- Qat_tbl_tmp$GCAM_region_ID[1]

      # Build a table for the broken out region based on Middle East data
      Mie_tbl[Mie_tbl == "Middle East"] <- reg_name
      Mie_tbl["GCAM_region_ID"][Mie_tbl["GCAM_region_ID"] == "21"] <- Qat_region_ID
      Qat_tbl <- Mie_tbl

      # Join new table to the original L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt table
      rbind(L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_tmp, Qat_tbl) -> L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new

      # Sort by GCAM_region_ID
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new[order(L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new$GCAM_region_ID),] -> L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt
    }

    # Handling Iraq case.
    L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
      filter(region == 'Iraq') -> TMP
    reg_name <- TMP$region[1]
    check_NA <- is.na(reg_name)
    ...
    if (check_NA == FALSE){
      ...
      # Get region ID for the broken out region
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt %>%
        filter(region == reg_name) -> Irq_tbl_tmp
      Irq_region_ID <- Irq_tbl_tmp$GCAM_region_ID[1]

      # Build a table for the broken out region based on Middle East data
      Mie_tbl[Mie_tbl == "Middle East"] <- reg_name
      Mie_tbl["GCAM_region_ID"][Mie_tbl["GCAM_region_ID"] == "21"] <- Irq_region_ID
      Irq_tbl <- Mie_tbl

      # Join new table to the original L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt table
      rbind(L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_tmp, Irq_tbl) -> L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new

      # Sort by GCAM_region_ID
      L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new[order(L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt_new$GCAM_region_ID),] -> L202.an_Prod_Mt_R_C_Sys_Fd_Y.mlt
    }

    # Adjusting the L202.ag_FeedCost_USDkg_R_F table
    # Handling Bahrain case.
    L202.ag_FeedCost_USDkg_R_F %>%
      filter(region == 'Bahrain') -> TMP
    reg_name <- TMP$region[1]
    check_NA <- is.na(reg_name)
    ...
    if (check_NA == FALSE){
      # Using Midlle East data as assumption for the broken out region.
      L202.ag_FeedCost_USDkg_R_F %>%
        filter(region != reg_name) -> L202.ag_FeedCost_USDkg_R_F_tmp
      L202.ag_FeedCost_USDkg_R_F %>%
        filter(region == 'Middle East') -> Mie_tbl
      ...
      # Join new table to the original L202.ag_FeedCost_USDkg_R_F table
      rbind(L202.ag_FeedCost_USDkg_R_F_tmp, Bah_tbl) -> L202.ag_FeedCost_USDkg_R_F_new

      # Sort by GCAM_region_ID
      L202.ag_FeedCost_USDkg_R_F_new[order(L202.ag_FeedCost_USDkg_R_F_new$region),] -> L202.ag_FeedCost_USDkg_R_F
    }

    # Handling Oman case.
    L202.ag_FeedCost_USDkg_R_F %>%
      filter(region == 'Oman') -> TMP
    reg_name <- TMP$region[1]
    check_NA <- is.na(reg_name)
    ...
    if (check_NA == FALSE){
      ...
      # Join new table to the original L202.ag_FeedCost_USDkg_R_F table
      rbind(L202.ag_FeedCost_USDkg_R_F_tmp, Omn_tbl) -> L202.ag_FeedCost_USDkg_R_F_new
    ...
    if (check_NA == FALSE){
      ...
      # Join new table to the original L202.ag_FeedCost_USDkg_R_F table
      rbind(L202.ag_FeedCost_USDkg_R_F_tmp, Qat_tbl) -> L202.ag_FeedCost_USDkg_R_F_new
    ...
    if (check_NA == FALSE){
      ...
      # Join new table to the original L202.ag_FeedCost_USDkg_R_F table
      rbind(L202.ag_FeedCost_USDkg_R_F_tmp, Irq_tbl) -> L202.ag_FeedCost_USDkg_R_F_new
