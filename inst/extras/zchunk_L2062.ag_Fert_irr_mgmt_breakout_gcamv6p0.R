...
module_aglu_L2062.ag_Fert_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    remove_glu_id <- glu_gcam$GLU %>% unique(); remove_glu_id
    regions_check <- GCAM_region_names$region[!GCAM_region_names$region %in% glu_gcam$region];  regions_check
    regions_check_id <- GCAM_region_names$GCAM_region_ID[!GCAM_region_names$GCAM_region_ID %in% glu_gcam$GCAM_region_ID];  regions_check_id
    ...
    L142.ag_Fert_IO_R_C_Y_GLU %>%
      dplyr::mutate(remove = dplyr::if_else(((GLU %in% remove_glu_id) & (GCAM_region_ID %in% regions_check_id)),1,0)) %>%
      dplyr::filter(remove==0) %>%
      dplyr::select(-remove) ->
      L142.ag_Fert_IO_R_C_Y_GLU
    ...
    L2062.AgCoef_Fert_ag_irr_mgmt %>%
      ...
      add_precursors("common/GCAM_region_names",
                     "water/basin_to_country_mapping","common/iso_GCAM_regID",
                     "L142.ag_Fert_IO_R_C_Y_GLU") ->
      L2062.AgCoef_Fert_ag_irr_mgmt
    L2062.AgCoef_Fert_bio_irr_mgmt %>%
