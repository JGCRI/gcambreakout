...
module_emissions_L112.ceds_ghg_en_R_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             ...
             FILE = "common/iso_GCAM_regID_Original_parent", # gcambreakout edit
    ...
    iso_GCAM_regID_Original_parent <- get_data(all_data, "common/iso_GCAM_regID_Original_parent")
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in% (L112.CEDS_GCAM_rice$GCAM_region_ID%>%unique())
    ]; missing_region_IDs

    for (missing_region_ID_i in missing_region_IDs){

      # Get a list of the original isos for the missing region
      missing_isos_i <- (iso_GCAM_regID %>%
                           dplyr::filter(GCAM_region_ID==missing_region_ID_i))$iso %>%
        unique() %>%
        sort(); missing_isos_i

      # Get the original GCAM_region_IDs for the missing iso's
      original_GCAM_region_IDs_i <- (iso_GCAM_regID_Original_parent %>%
        dplyr::filter(iso %in% missing_isos_i))$GCAM_region_ID %>%
        unique() %>%
        sort(); original_GCAM_region_IDs_i

      # Now get the average value of the original GCAM_region_IDs for L112.CEDS_GCAM_rice
      # And assume this value for the new missing region
      L112.CEDS_GCAM_rice %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Non.CO2,CEDS_agg_sector,CEDS_agg_fuel,year) %>%
        dplyr::summarize(emissions = mean(emissions, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Non.CO2,CEDS_agg_sector,CEDS_agg_fuel,year, emissions) ->
        L112.CEDS_GCAM_rice_append

      L112.CEDS_GCAM_rice %>%
        dplyr::bind_rows(L112.CEDS_GCAM_rice_append) -> L112.CEDS_GCAM_rice
    }
    ...
    L122.ag_Prod_Mt_R_rice_Y_GLU %>%
      ...
      left_join(L112.CEDS_GCAM_rice, by = c("GCAM_region_ID", "Non.CO2", "year")) %>%
    ...
    # Identify missing regions
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in% (L112.CEDS_GCAM_soil$GCAM_region_ID%>%unique())
    ]; missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L112.CEDS_GCAM_soil %>%
        ...
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Non.CO2,CEDS_agg_sector,CEDS_agg_fuel,year, emissions) ->
        L112.CEDS_GCAM_soil_append

      L112.CEDS_GCAM_soil %>%
        dplyr::bind_rows(L112.CEDS_GCAM_soil_append) -> L112.CEDS_GCAM_soil
    ...
    # Identify missing regions
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L121.nonco2_tg_R_awb_C_Y_GLU %>% dplyr::filter(!is.na(value)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs

    missing_region_IDs <- missing_region_IDs[missing_region_IDs > 32]
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L121.nonco2_tg_R_awb_C_Y_GLU %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Non.CO2,GCAM_commodity,GCAM_subsector, GLU, year) %>%
        dplyr::summarize(value = mean(value, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Non.CO2,GCAM_commodity,GCAM_subsector, GLU, year, value) ->
        L121.nonco2_tg_R_awb_C_Y_GLU_append

      L121.nonco2_tg_R_awb_C_Y_GLU %>%
        dplyr::bind_rows(L121.nonco2_tg_R_awb_C_Y_GLU_append) -> L121.nonco2_tg_R_awb_C_Y_GLU
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L124.nonco2_tg_R_grass_Y_GLU %>% dplyr::filter(!is.na(value)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L124.nonco2_tg_R_grass_Y_GLU %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Land_Type, GLU, year, Non.CO2) %>%
        dplyr::summarize(value = mean(value, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Land_Type, GLU, year, Non.CO2, value) ->
        L124.nonco2_tg_R_grass_Y_GLU_append

      L124.nonco2_tg_R_grass_Y_GLU %>%
        dplyr::bind_rows(L124.nonco2_tg_R_grass_Y_GLU_append) -> L124.nonco2_tg_R_grass_Y_GLU
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L124.nonco2_tg_R_forest_Y_GLU %>% dplyr::filter(!is.na(value)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs

    missing_region_IDs <- missing_region_IDs[missing_region_IDs > 32];missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L124.nonco2_tg_R_forest_Y_GLU %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Land_Type, GLU, year, Non.CO2, technology) %>%
        dplyr::summarize(value = mean(value, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Land_Type, GLU, year, Non.CO2, technology, value) ->
        L124.nonco2_tg_R_forest_Y_GLU_append

      L124.nonco2_tg_R_forest_Y_GLU %>%
        dplyr::bind_rows(L124.nonco2_tg_R_forest_Y_GLU_append) -> L124.nonco2_tg_R_forest_Y_GLU
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L131.nonco2_tg_R_prc_S_S_Yh %>% dplyr::filter(!is.na(value)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L131.nonco2_tg_R_prc_S_S_Yh %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(supplysector, subsector, stub.technology, Non.CO2, year) %>%
        dplyr::summarize(value = mean(value, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,supplysector, subsector, stub.technology, Non.CO2, year, value) ->
        L131.nonco2_tg_R_prc_S_S_Yh_append

      L131.nonco2_tg_R_prc_S_S_Yh %>%
        dplyr::bind_rows(L131.nonco2_tg_R_prc_S_S_Yh_append) -> L131.nonco2_tg_R_prc_S_S_Yh
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L125.bcoc_tgbkm2_R_grass_2000 %>% dplyr::filter(!is.na(em_factor)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L125.bcoc_tgbkm2_R_grass_2000 %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Land_Type, Non.CO2, year) %>%
        dplyr::summarize(em_factor = mean(em_factor, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Land_Type, Non.CO2, year, em_factor) ->
        L125.bcoc_tgbkm2_R_grass_2000_append

      L125.bcoc_tgbkm2_R_grass_2000 %>%
        dplyr::bind_rows(L125.bcoc_tgbkm2_R_grass_2000_append) -> L125.bcoc_tgbkm2_R_grass_2000
    ...
    missing_region_IDs <- (iso_GCAM_regID$GCAM_region_ID%>%unique())[
      !(iso_GCAM_regID$GCAM_region_ID%>%unique()) %in%
        ((L125.bcoc_tgbkm2_R_forest_2000 %>% dplyr::filter(!is.na(em_factor)))$GCAM_region_ID%>%unique())
    ]; missing_region_IDs
    ...
    for (missing_region_ID_i in missing_region_IDs){
      ...
      L125.bcoc_tgbkm2_R_forest_2000 %>%
        dplyr:: filter(GCAM_region_ID %in% original_GCAM_region_IDs_i) %>%
        dplyr::select(-GCAM_region_ID) %>%
        dplyr::group_by(Land_Type, Non.CO2, technology) %>%
        dplyr::summarize(em_factor = mean(em_factor, na.rm=T)) %>%
        dplyr::ungroup() %>%
        dplyr::mutate(GCAM_region_ID = missing_region_ID_i) %>%
        dplyr::select(GCAM_region_ID,Land_Type, Non.CO2, technology, em_factor) ->
        L125.bcoc_tgbkm2_R_forest_2000_append

      L125.bcoc_tgbkm2_R_forest_2000 %>%
        dplyr::bind_rows(L125.bcoc_tgbkm2_R_forest_2000_append) -> L125.bcoc_tgbkm2_R_forest_2000
