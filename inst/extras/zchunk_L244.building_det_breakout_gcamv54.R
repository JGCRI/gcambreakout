...
module_energy_L244.building_det <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    ### SRSdS, 03Sep21: Handling Bahrain case when Bahrain is broken out as an individual region
    # Get Bahrain's GCAM_region_ID
    GCAM_region_names %>%
      filter(region == 'Bahrain') -> TMP
    bhr_ID <- TMP$GCAM_region_ID[1]
    check_NA <- is.na(bhr_ID)

    if (check_NA == FALSE){ # Only execute these lines if Bahrain is broken out as an individual region
      L244.HDDCDD_scen_R_Y %>%
        filter(region == 'Qatar') -> TMP
      reg_name <- TMP$region[1]
      reg_ID_2   <- TMP$GCAM_region_ID[1]
      check_NA_2 <- is.na(reg_name)

      L244.HDDCDD_scen_R_Y %>%
        filter(region == 'Saudi Arabia') -> TMP
      reg_name <- TMP$region[1]
      reg_ID_3   <- TMP$GCAM_region_ID[1]
      check_NA_3 <- is.na(reg_name)

      L244.HDDCDD_scen_R_Y %>%
        filter(region == 'Middle East') -> TMP
      reg_name <- TMP$region[1]
      reg_ID_4   <- TMP$GCAM_region_ID[1]
      check_NA_4 <- is.na(reg_name)

      if (check_NA_4 == FALSE){
        region_to_filter <- "Middle East"
        reg_ID <- reg_ID_4
      }
      if (check_NA_3 == FALSE){
        region_to_filter <- "Saudi Arabia"
        reg_ID <- reg_ID_3
      }
      if (check_NA_2 == FALSE){
        region_to_filter <- "Qatar"
        reg_ID <- reg_ID_2
      }

      # As L244.HDDCDD_scen_R_Y does not contain Bahrain, use data from region_to_filter as assumption for Bahrain.
      L244.HDDCDD_scen_R_Y %>%
        filter(region == region_to_filter) -> tbl_filtered
      tbl_filtered["GCAM_region_ID"][tbl_filtered["GCAM_region_ID"] == reg_ID] <- bhr_ID
      tbl_filtered["region"][tbl_filtered["region"] == region_to_filter] <- "Bahrain"

      # Join new Bahrain table with L244.HDDCDD_scen_R_Y
      rbind(L244.HDDCDD_scen_R_Y, tbl_filtered) -> L244.HDDCDD_scen_R_Y
    }
    ###
