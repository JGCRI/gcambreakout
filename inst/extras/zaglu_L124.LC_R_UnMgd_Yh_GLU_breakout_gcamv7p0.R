...
module_aglu_L124.LC_R_UnMgd_Yh_GLU <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    ## ksa edit (NTG 09/29/2023) KSA shows negative values of managed forests and is missing
    # values for unmanaged forests before 1900. As these are small values we will zero them
    # out here. Can be revisited in the future. Note: this only includes softwood forests as
    # hardwood forests do not have data in KSA right now.
    L120.LC_bm2_R_LT_Yh_GLU %>%
      filter(Land_Type %in% c(aglu.FOREST_NODE_NAMES)) %>%
      rename(TotForest = value) %>%
      ## NTG: we change the line below to keep the NA's prior to 1900 in KSA which
      ## we can then zero out, as well as zeroing out the negative managed forest values.
      #left_join_error_no_match(L123.LC_bm2_R_MgdFor_Yh_GLU, by = c("GCAM_region_ID", "GLU", "Land_Type", "year")) %>%
      left_join(L123.LC_bm2_R_MgdFor_Yh_GLU, by = c("GCAM_region_ID", "GLU", "Land_Type", "year")) %>%
      rename(MgdForest = value) %>%
      mutate(MgdForest = if_else(MgdForest<0, 0, MgdForest)) %>%
