...
module_aglu_LB1321.regional_ag_prices <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    ### SRSdS, 14Nov21 - Handling Gabon case. Gabon is not listed in "aglu/FAO/FAO_ag_an_ProducerPrice". To break out
    # Gabon as an individual region, some assumption are necessary to handle this lack of data. Gabon is inheriting
    # assumptions from its parental region, Africa_Western.
    # Get GCAM_Region_ID of Gabon to test if the region has been broken out
    iso_GCAM_regID %>%
      filter(iso == 'gab') -> gab_tbl
    gab_Region_ID <- gab_tbl$GCAM_region_ID[1]

    # Lines below are only processed if Gabon has been broken out as an individual region
    if (gab_Region_ID != 5){
      # Get assumptions from the parental region, Africa_Western
      L1321.ag_prP_R_C_75USDkg %>%
        filter(GCAM_region_ID == 5) -> afw_tbl
      afw_tbl["GCAM_region_ID"][afw_tbl["GCAM_region_ID"] == 5] <- gab_Region_ID
      gab_tbl_ag <- afw_tbl
      ...
      rbind(L1321.ag_prP_R_C_75USDkg, gab_tbl_ag) -> L1321.ag_prP_R_C_75USDkg
      rbind(L1321.an_prP_R_C_75USDkg, gab_tbl_an) -> L1321.an_prP_R_C_75USDkg_new
