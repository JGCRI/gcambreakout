...
#' @importFrom dplyr bind_rows filter full_join group_by inner_join left_join mutate select summarise ungroup count
...
module_water_L171.desalination <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Note: (zk 6 Aug 2021) Making this generic for any efw.COUNTRIES_ELEC_DESAL that is broken out as a new region
    efw.COUNTRIES_ELEC_DESAL_IDs <- (iso_GCAM_regID %>%
      filter(iso %in% efw.COUNTRIES_ELEC_DESAL))$GCAM_region_ID %>% unique()

    efw.COUNTRIES_ELEC_DESAL_OWN_REGION_IDs <- (iso_GCAM_regID %>%
      filter(GCAM_region_ID %in% efw.COUNTRIES_ELEC_DESAL_IDs) %>%
      dplyr::count(GCAM_region_ID) %>%
      filter(n==1))$GCAM_region_ID %>% unique()

    efw.COUNTRIES_ELEC_DESAL_OWN_REGION <- (iso_GCAM_regID %>%
      filter(GCAM_region_ID %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION_IDs))$iso%>%unique()


    L171.out_km3_R_desalfromelec_Yh <- subset(L171.out_km3_ctry_desal_Yh, iso %in% efw.COUNTRIES_ELEC_DESAL ) %>%
      mutate(value = if_else(iso %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION, value * efw.SAU_DesalFromElecFrac, value)) %>%
    ...
    L171.out_km3_ctry_desal_Yh <- L171.out_km3_ctry_desal_Yh %>%
      mutate(value = if_else(iso %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION, value * (1 - efw.SAU_DesalFromElecFrac), value),
             value = if_else(iso %in% efw.COUNTRIES_ELEC_DESAL & !(iso %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION), 0, value))
    ...
    L171.out_km3_ctry_desal_tech_Yh <- repeat_add_columns(L171.out_km3_ctry_desal_Yh,
      ...
      mutate(share = if_else(iso %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION & technology == "distillation", 1 - efw.SAU_DesalOnlyROFrac, share),
             share = if_else(iso %in% efw.COUNTRIES_ELEC_DESAL_OWN_REGION & technology == "reverse osmosis", efw.SAU_DesalOnlyROFrac, share),
             value = value * share) %>%
