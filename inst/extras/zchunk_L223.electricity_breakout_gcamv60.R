...
module_energy_L223.electricity <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Adding in missing countries and regions as 0 to L120.RegCapFactor_offshore_wind
    # Mostly these regions have no off_shore wind
    L120.RegCapFactor_offshore_wind_missing_gcam_regions <-
      unique(GCAM_region_names$region)[!unique(GCAM_region_names$region) %in%
                                         unique(L120.RegCapFactor_offshore_wind$region)]

    missing_regions_df <- data.frame(CF = 0, CFmax = 0) %>%
      merge(GCAM_region_names %>%
              dplyr::filter(region %in% L120.RegCapFactor_offshore_wind_missing_gcam_regions))
